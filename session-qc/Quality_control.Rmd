...
The datasets are available on the classroom computers at `~/scrna-seq2019/day1/1-qc/`
...
You can either create an SCE object directly from the count matrices, have a look at their tutorial at: https://bioconductor.org/packages/release/bioc/vignettes/scater/inst/doc/vignette-intro.html. 
...
As you can see, MALAT1 corresponds to an average of around 4% of the counts. And in some cells as high as ~30% of the counts. I would consider removing that gene before further analysis and clustering. Also, the mitochondrial genes correspond to a high proportion of the total counts and should probably be removed.
...
The function plotRowData can plot any of the stats in rowData, for instance mean expression vs number of cells with detection.
...
Note! You can also run PCA/tSNE etc in Seurat and they will automatically be imported into the SCE object. 
...
Each line corresponds to one factor and represents the distribution of R-squared values across all genes. 
